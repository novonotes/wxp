...
use novonotes_run_loop::{Handle, RunLoopLocal};

trait TimerCallback: 'static {
    fn execute(&self, run_loop: &RunLoopLocal);
}
...
where
    F: Fn() + 'static,
{
    fn execute(&self, _run_loop: &RunLoopLocal) {
        (self.callback)();
    }
}
...
impl Timer {
    ...
    ///
    /// This keeps per-timer mutable state next to the callback without requiring callers to
    /// allocate their own `Rc<RefCell<_>>` wrapper.
    ...
    ///
    /// The [`RunLoopLocal`] argument makes the run-loop-thread requirement explicit.
    pub fn start(&self, run_loop: &RunLoopLocal) {
        // Re-scheduling when already running would double the tick rate; bail out.
        if self.inner.is_running.replace(true) {
            return;
        }

        self.schedule_next(run_loop);
    }
    ...
    fn schedule_next(&self, run_loop: &RunLoopLocal) {
        ...
        let handle = run_loop.schedule(interval, move |run_loop| {
            run_timer_tick(run_loop, weak_inner);
        });
...
fn run_timer_tick(run_loop: &RunLoopLocal, weak_inner: Weak<TimerInner>) {
    ...
    let handle = run_loop.schedule(interval, move |run_loop| {
        run_timer_tick(run_loop, weak_inner_for_next);
    });
...
mod tests {
    ...
    fn stop_prevents_future_ticks_after_observed_fire() {
        ...
        run_loop.block_on(async {
            ...
            timer.start(run_loop);
            run_loop.delay(Duration::from_millis(150)).await;
            timer.stop();
            let count_at_stop = counter.load(Ordering::SeqCst);
            run_loop.delay(Duration::from_millis(200)).await;
    ...
    fn async_task_continues_after_timer_drop() {
        ...
        run_loop.block_on(async {
...

            {
                ...
                timer.start(run_loop);
                wait_until_true(run_loop, &task_started).await;
            }
            ...
            wait_until_true(run_loop, &task_completed).await;
        });
    }

    async fn wait_until_true(run_loop: &RunLoopLocal, flag: &AtomicBool) {
        ...
        for _ in 0..50 {
            ...
            run_loop.delay(Duration::from_millis(20)).await;
        }
