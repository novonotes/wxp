...
`init()` は現在のスレッドを RunLoop スレッドとしてマークし、そのスレッド上のネイティブループ基盤への参照を取得（存在しない場合は新規作成）します。スタンドアロンアプリでは UI/run loop を駆動するスレッドで呼び、その後に自前で `run()` を呼んでランループを駆動します。

スレッド binding は少なくとも 1 つの `RunLoopGuard` が生きている間だけ有効です。最後の guard を drop すると run loop は shutdown され、binding も解除されます。その後の `init()` では別スレッドに新しい run loop を bind できます。

プラグイン環境では、GUI コールバックを受けるホストの main/UI スレッドで `init()` を呼びます。CLAP の `clap_entry.init` で呼ぶ API として扱わないでください。`clap_entry.init` は DSO 初期化用であり、高速であるべきで、スキャン用スレッドや worker thread から呼ばれる可能性があります。プラグイン環境ではホストが既にネイティブループを駆動しているため、`run()` の呼び出しは不要です。その後のコールバックやタイマー登録は、誰がループを駆動しているかに関係なく同じように動作します。
...
run_loop を利用したコードのテスト用ヘルパーや GUI テストハーネスは、関連クレート `run_loop_test_utils` で提供しています。
使い方については、[`run_loop_test_utils` README](../run_loop_test_utils/README.md)を参照してください。
