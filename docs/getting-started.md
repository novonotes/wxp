...
`wxp-examples` の `examples/gain_plugin` をテンプレートとして新しい wxp プラグインを作成する手順を説明します。
...
このガイドでは `wxp` と `wxp-examples` を使って新しいプラグインを作成する方法を説明します。
まず両方のリポジトリをクローンしてください。

```sh
git clone https://github.com/novonotes/wxp.git
git clone https://github.com/novonotes/wxp-examples.git
```

### 2. テンプレートをコピー

`wxp-examples/examples/gain_plugin` をそのままコピーします。

```sh
cp -r /path/to/wxp-examples/examples/gain_plugin /path/to/my_plugin
```
...
CLAP_WRAPPER_DIR=/path/to/wxp-examples/clap_wrapper_builder \
...
example の gain plugin の [README.md](https://github.com/novonotes/wxp-examples/blob/main/examples/gain_plugin/README.md) を読んでみましょう。
