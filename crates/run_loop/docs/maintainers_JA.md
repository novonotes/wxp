...
`RunLoop::init()` は CLAP の `clap_entry.init` と同じ lifecycle hook ではありません。
CLAP entry の初期化は DSO 初期化用であり、高速であるべきで、スキャン用スレッドや
worker thread から呼ばれる可能性があります。一方 `RunLoop::init()` は現在のスレッドを
